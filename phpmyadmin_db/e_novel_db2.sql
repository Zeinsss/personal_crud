...
--
-- Host: 127.0.0.1
-- Generation Time: Mar 24, 2024 at 04:36 AM
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `user_view`  AS SELECT `u`.`id` AS `Id`, `u`.`name` AS `Username`, `u`.`email` AS `Email`, `n`.`name` AS `Favorite_Novel` FROM ((`users` `u` join `favorite` `f` on(`u`.`id` = `f`.`user_id`)) join `novel` `n` on(`n`.`id` = `f`.`novel_id`)) ORDER BY `u`.`id` ASC ;
